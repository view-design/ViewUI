<template>
	<div class="QAnchor-wrapper">
		<div class="link-wrapper">
            <QButton @click="changeCon">修改为Window</QButton>
            <QButton @click="andLink">添加一个连接</QButton>
			<QAnchor :bounds="100" @on-change="handleChange" @on-select="handleSelect" :style="{right: '100px'}" :affix="true" :offset-top="30" :scroll-offset="100" :container="scrollCon" show-ink>
				<QAnchorLink v-if="(link - 1) % 30 === 0" v-for="link in 300" :key="`link${link}`" :href="`#title-${link}`" :title="`title-${link}`">
                    <QAnchorLink :scroll-offset="20" v-if="link === 61" href="#title-child-69" title="title-child-69"/>
                </QAnchorLink>
                <QAnchorLink :scroll-offset="200" href="#lishi" title="李氏专跳"/>
                <QAnchorLink v-if="showNewLink" href="#new-link" title="这是动态添加的连接"/>
			</QAnchor>
		</div>
		<div v-if="con === 'div'" ref="listWrapper" id="listWrapper" class="list-wrapper">
            ...
            <template v-for="i in 300">
                ...
                <QCollapse v-if="i === 3" v-model="value1" :key="`QCollapse-${i}`">
                    <QPanel name="1">
                        史蒂夫·乔布斯
